...
import PreprocessingPopup, { preprocessingPopups } from '../../../components/PreprocessingPopups';
import { motion } from "framer-motion";

type PreprocessingOption = 'median' | 'remove' | 'labelEncoding' | 'oneHot' | 'normalized' | 'standardized';
...
const Preprocessing: FC = () => {
  ...
  const [selectedOptions, setSelectedOptions] = useState<PreprocessingState>(initialState);
  const [showResults, setShowResults] = useState(false);
  const [activeModal, setActiveModal] = useState<PreprocessingOption | null>(null);
  ...
  const getHexagonColor = (option: string) => {
    if (!showResults) {
      return selectedOptions[option] ? 'from-[#66c0f4] via-[#4fa3e3] to-[#3c8bbd]' : 'from-[#1a2332] via-[#243647] to-[#2c4159]';
    }
    if (correctAnswers[option as keyof typeof correctAnswers]) {
      return selectedOptions[option] ? 'from-green-500 via-green-600 to-green-700' : 'from-[#1a2332] via-[#243647] to-[#2c4159]';
    }
    return selectedOptions[option] ? 'from-red-500 via-red-600 to-red-700' : 'from-[#1a2332] via-[#243647] to-[#2c4159]';
  };
  ...
  const getFinalPath = () => {
    ...
    if (selectedOptions.oneHot) path.push('OneHot');
    if (selectedOptions.normalized) path.push('Normalized');
    return path.join(' → ');
  };

  const isCorrectPath = () => {
    return selectedOptions.median && selectedOptions.oneHot && selectedOptions.normalized &&
           !selectedOptions.remove && !selectedOptions.labelEncoding && !selectedOptions.standardized;
  };
  ...
  const Hexagon = ({ option, label }: { option: PreprocessingOption; label: string }) => (
    <button
      onClick={() => handleOptionClick(option)}
      className="relative transform hover:scale-105 transition-all duration-300"
    >
      <div className={`w-36 h-36 relative ${selectedOptions[option] ? 'animate-glow' : ''}`}>
        <div className={`absolute inset-0 bg-gradient-to-br ${getHexagonColor(option)} 
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <span className={`text-lg font-bold ${getOptionColor(option)} text-center font-mono tracking-wider`}>
              {label}
            </span>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto p-4">
        ...
        <div className="mb-12">
          ...
          <div className="prose prose-invert space-y-4 font-mono">
            ...
            <p>
              <strong className="text-[#66c0f4]">📊 NeoVerse Data Preview:</strong>
            </p>
          </div>
          ...
          <div className="mt-12 text-center space-y-6">
            ...
            {showResults && (
              <div className={`p-6 rounded-lg ${
                isCorrectPath() ? 'bg-[#2c4159] border-green-500/30' : 'bg-[#2c2332] border-red-500/30'
              } border-2 max-w-2xl mx-auto font-mono`}>
                <p className="text-xl mb-4">
                  Selected Path: <span className="text-[#66c0f4]">{getFinalPath()}</span>
                </p>
                <p className={`text-lg ${isCorrectPath() ? 'text-green-400' : 'text-red-400'}`}>
                  {isCorrectPath()
